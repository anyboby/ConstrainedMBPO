params = {
    ...
    'log_dir': '~/ray_mbpo/',
    'exp_name': 'defaults',
    'use_mjc_state_model': False,       # not nice, have to set two flags for the same thing
    ...
    'kwargs': {
        ...
        'n_train_repeat': 20, #20 #40,      # -> refers to total timesteps
        ...
        'rollout_batch_size': 100e3,    # rollout_batch_size is the size of randomly chosen states to start from when rolling out model
        'deterministic': False,                     
        'num_networks': 7,              # size of model network ensemble
        'num_elites': 5,                # best networks to select from num_networks
        'real_ratio': 0.3,#0.05,        # ratio to which the training batch for the rl_algo is composed
        'target_entropy': -3, 
        'max_model_t': None,            # a timeout for model training (e.g. for speeding up wallclock time)
        'rollout_schedule': [150, 300, 1, 20],   # min_epoch, max_epoch, min_length, max_length = self._rollout_schedule
