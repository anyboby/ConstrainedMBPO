...
class ModelSamplerDeb(CpoSampler):
    def __init__(self,
                 max_path_length,
                 batch_size=1000,
    ...
    def sample(self):
        ...
        get_action_outs = self.policy.get_action_outs(current_obs, factored=True)
        ...
        next_obs, reward, terminal, info = self.env.step_deb(current_obs, a)
        ...
        if not self.use_inv_var:                ### only need to append vals if 
            ...
            if append_vals:
                if self.policy.agent.reward_penalized:
                    last_val = np.squeeze(np.mean(self.policy.get_v(cur_obs[term_mask], factored=True), axis=0))

                else:
                    last_val = np.squeeze(np.mean(self.policy.get_v(cur_obs[term_mask], factored=True), axis=0))
                    last_cval = np.squeeze(np.mean(self.policy.get_vc(cur_obs[term_mask], factored=True), axis=0))
    ...
    def finish_all_paths(self):
        ...
        if alive_paths.any():
            ...
            if not self.use_inv_var:
                ...
                term_mask = np.ones(shape=alive_paths.sum(), dtype=np.bool)
                if self.policy.agent.reward_penalized:
                    last_val = np.squeeze(np.mean(self.policy.get_v(self._current_observation, factored=True), axis=0))
                else:
                    last_val = np.squeeze(np.mean(self.policy.get_v(self._current_observation, factored=True), axis=0))
                    last_cval = np.squeeze(np.mean(self.policy.get_vc(self._current_observation, factored=True), axis=0))
